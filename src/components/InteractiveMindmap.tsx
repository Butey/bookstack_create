...
export const InteractiveMindmap = React.memo(function InteractiveMindmap({ data }: InteractiveMindmapProps) {
  ...
  useEffect(() => {
    ...
    type ExtendedHierarchyNode = d3.HierarchyNode<MindMapNode> & {
      x0?: number;
      y0?: number;
    ...
    const diagonal = d3.linkHorizontal<d3.HierarchyPointLink<MindMapNode>, d3.HierarchyPointNode<MindMapNode>>()
        .x(d => d.y)
        .y(d => d.x);
